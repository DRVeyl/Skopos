  ...
  public class Monitor {
    ...
    public void AlertIfNeeded() {
      ...
      if (metric_.availability >= availability_threshold_) {
        if (alerted_ && (telecom.last_universal_time > last_restore_time_ + telecom.alert_rate_limit_ * 86400)) {
          alerted_ = false;
        ...
        if (!alerted_ && (telecom.last_universal_time > last_alert_time_ + telecom.alert_rate_limit_ * 86400)) {
          alerted_ = true;
          last_alert_time_ = telecom.last_universal_time;
          if (telecom.stop_warp_in_sim || !RP0.SpaceCenterManagement.Instance.IsSimulatedFlight) {
